...
<div class="card shadow mb-4 mx-5">
    ...
    <div class="card-body">
        <form class="user" action="{{route('subcategoria.update',['subcategorium'=>$subcategoria->codigo_subcat])}}" method="post">
            @csrf
            @method('put')
            <div class="form-group row">
                <div class="col-sm-6">
                    <input type="text" name="nombre_subcat" class="form-control form-control-user" id="exampleLastName"
                    placeholder="Nombre" value="{{$subcategoria->nombre_subcat}}">
                </div>
                    ...
                    ">
                        ...
                        @foreach ($categorias as $categoria)
                            <option value="{{$categoria->codigo_cat}}"
                                @if ($categoria->codigo_cat == $subcategoria->codigo_cat)
                                    selected
                                @endif
                                >
            ...
            <div class="form-group">
                <input type="text" name="descri_subcat" class="form-control form-control-user" id="exampleInputEmail"
                    placeholder="descripcion" value="{{$subcategoria->descri_subcat}}">
            </div>
